...
func _process(_delta):
	if visible != last_visible:
		print("TextUI.gd: visible = ", visible, "; at ", Time.get_ticks_msec(), "; path: ", get_path())
		last_visible = visible

func show_area_name(area_name: String):
	# Cancel previous timer if it exists and is still running
	if area_timer and area_timer.is_connected("timeout", _hide_area_name_box):
		area_timer.timeout.disconnect(_hide_area_name_box)
		area_timer = null
	...
	area_timer.timeout.connect(_hide_area_name_box)

func _hide_area_name_box():
	#DEBUG print("TextUI.gd/hide_area_name_box called")
	area_name_box.visible = false

func show_dialogue_box(text: String):
	print("TextUI.show_dialogue_box called with text: ", text)
	dialogue_box.show()
	dialogue_text.text = text
	if waiting_for_confirm:
		print("Already waiting for confirm; ignoring extra call.")
		return
...
func hide_dialogue_box():
	...
	dialogue_box.visible = false
	waiting_for_confirm = false
	print("Hiding TextUI from ", get_script().resource_path, "; at ", Time.get_ticks_msec())
	print("TextUI visibility after change: ", self.visible, self.get_path())

func _wait_for_confirm():
	print("TextUI.gd/_wait_for_confirm() called")
